...
class LinkedList(object):
    ...
    def length(self):
        ...
        Best and worst case running time: O(1) under all conditions. """
    ...
    def insert_at_index(self, index, item):
        ...
        raise ValueError if the given index is out of range of the list size.
        Best case running time: O(1) if index is first or last in the list
        Worst case running time: O(n-1) if index is secon to last in the list"""
        # Check if the given index is out of range and if so raise an error
        if not (0 <= index <= self.size): 
            raise ValueError('List index out of range: {}'.format(index))
        ...
        if index == 0:
            ...
            while node is not None:
                if node.data == item:
                    ...
                    new_node.next = node.next # Link new to current's next
                    previous.next = new_node # Relink previous to new
                    node.next = None
                    node = new_node
                previous = node
                node = node.next
